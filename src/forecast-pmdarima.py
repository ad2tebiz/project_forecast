...
def create_directories():
    ...
    os.makedirs('data/pmdarima', exist_ok=True)
...
def plot_original_data(data):
    ...
    plt.tight_layout()
    plt.savefig('data/pmdarima/original_data_plot.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def save_forecast_to_csv(forecast_df):
    ...
    try:
        ...
        forecast_df.to_csv('data/pmdarima/forecast_results.csv', index=True)
        print(f"Прогноз сохранен в 'data/pmdarima/forecast_results.csv'")
        
        # Дополнительно сохраняем краткую версию
        forecast_df[['predicted_value', 'confidence_lower', 'confidence_upper']].to_csv(
            'data/pmdarima/forecast_simple.csv'
        )
...
def main():
    ...
    print(f"   - data/pmdarima/forecast_results.csv")
    print(f"   - data/pmdarima/forecast_simple.csv") 
    print(f"   - data/pmdarima/original_data_plot.png")
    print(f"   - data/pmdarima/forecast_plot.png")
