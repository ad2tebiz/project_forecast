...
def create_directories():
    ...
    os.makedirs('data/statsmodels', exist_ok=True)
...
def plot_original_data(df):
    ...
    plt.tight_layout()
    plt.savefig('data/statsmodels/original_data_plot.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_sarima_results(df, model, forecast, confidence_int, future_dates):
    ...
    plt.tight_layout()
    plt.savefig('data/statsmodels/sarima_forecast_plot.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_sarima_diagnostics(model):
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig('data/statsmodels/sarima_diagnostics.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def save_forecast_to_csv(forecast_df, forecast, future_dates):
    ...
    try:
        ...
        forecast_df.to_csv('data/statsmodels/sarima_forecast_results.csv', index=True)
        print("Прогноз SARIMA сохранен в 'data/statsmodels/sarima_forecast_results.csv'")
        ...
        simple_forecast.to_csv('data/statsmodels/sarima_simple_forecast.csv', index=False)
        print("Упрощенный прогноз SARIMA сохранен в 'data/statsmodels/sarima_simple_forecast.csv'")
...
def main():
    ...
    print(f"   - data/statsmodels/sarima_forecast_results.csv (полный прогноз)")
    print(f"   - data/statsmodels/sarima_simple_forecast.csv (упрощенный прогноз)")
    print(f"   - data/statsmodels/original_data_plot.png")
    print(f"   - data/statsmodels/sarima_forecast_plot.png")
    print(f"   - data/statsmodels/sarima_diagnostics.png")
